...
interface RCMMetricsCSVUploadProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const RCMMetricsCSVUpload: React.FC<RCMMetricsCSVUploadProps> = ({
  isOpen,
  onClose,
  onSuccess
}) => {
  ...
  const parseCSV = async (file: File) => {
    ...
    try {
      ...
      const valueIndex = headers.findIndex(h => h === 'value' || h.includes('amount') || h.includes('count'));
      ...
      if (dateIndex === -1 || keyIndex === -1 || valueIndex === -1) {
        ...
        if (valueIndex === -1) missing.push('value (or count)');
...
${today},rcm_pending_claims,0,Claims with pending status
${today},rcm_denied_claims,0,Claims with denied status`;

    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `rcm-metrics-template-${today}.csv`;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Upload RCM Metrics CSV</h2>
            <p className="text-sm text-gray-600 mt-1">
              Upload a CSV file with daily RCM metric values (claims over 60 days, active claims, etc.)
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              ...
              <li>• field_key options: rcm_claims_over_60_days, rcm_active_claims, rcm_pending_claims, rcm_denied_claims</li>
